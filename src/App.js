      ...
      {ajout && (
        <div style={carteStyle}>
          ...
          {form.equipe.length > 0 && (
            <table className="table-cards" style={{ width: '100%', marginBottom: '10px' }}>
              <thead><tr>
                {['Employé', 'Rôle', 'Jours prévus', ...(form.id ? ['Jours réalisés'] : []), 'Coût prévu', ''].map(h => <th key={h} style={thStyle}>{h}</th>)}
              </tr></thead>
              <tbody>
                {form.equipe.map((m, i) => {
                  const emp = parametres.employes.find(e => e.id === parseInt(m.employeId));
                  return <tr key={m._uid || `equipe-${m.employeId}-${m.role}`}>
                    <td style={tdStyle}>{emp?.nom}</td>
                    <td style={tdStyle}>{m.role}</td>
                    <td style={tdStyle}>{m.joursPlannifies}j</td>
                    {form.id && (() => {
                      const empId = parseInt(m.employeId);
                      const heuresReel = heuresEmploye(form.journal || [], empId);
                      const reel = heuresReel / 8;
                      const prevu = parseFloat(m.joursPlannifies) || 0;
                      const depasse = prevu > 0 && reel > prevu;
                      const couleurReel = depasse ? C.warning : reel > 0 ? 'var(--text-secondary)' : 'var(--text-muted)';
                      const tooltip = heuresReel > 0
                        ? `${Math.round(heuresReel * 10) / 10}h réalisées`
                        : '';
                      return (
                        <td style={tdStyle} title={tooltip}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                            <span style={{ fontWeight: 600, fontSize: 14, color: couleurReel, minWidth: 32, textAlign: 'right' }}>
                              {heuresReel > 0 ? `${Math.round(heuresReel * 10) / 10}h` : <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>—</span>}
                            </span>
                            {reel > 0 && (
                              <span style={{ fontSize: 10, color: 'var(--text-muted)', opacity: 0.7 }}>/ {prevu}j</span>
                            )}
                            {depasse && (
                              <span style={{ fontSize: 10, fontWeight: 700, color: C.warning, background: C.warning + '18', border: `1px solid ${C.warning}40`, padding: '2px 5px', borderRadius: 4, whiteSpace: 'nowrap' }}>
                                +{(reel - prevu).toFixed(1)}j
                              </span>
                            )}
                          </div>
                          {heuresReel > 0 && (
                            <div style={{ fontSize: 10, color: 'var(--text-muted)', opacity: 0.6, marginTop: 2 }}>{Math.round(reel * 10) / 10}j réalisés</div>
                          )}
                        </td>
                      );
                    })()}
                    <td style={tdStyle}>CHF {fmtN((emp?.tarifJour || 0) * m.joursPlannifies)}</td>
                    <td style={tdStyle}><button onClick={() => setForm({ ...form, equipe: form.equipe.filter((_, idx) => idx !== i) })} style={btnDanger}>🗑️</button></td>
                  </tr>;
                })}
              </tbody>
            </table>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr auto', gap: '10px', alignItems: 'end', marginBottom: '20px' }}>
            <div><label style={labelStyle}>Employé</label>
              <select value={membreEquipe.employeId} onChange={e => {
                const emp = parametres.employes.find(emp => emp.id === parseInt(e.target.value));
                setMembreEquipe({ ...membreEquipe, employeId: e.target.value, role: emp?.poste || membreEquipe.role });
              }} style={inputStyle}>
                <option value="">Sélectionner...</option>
                {parametres.employes.map(e => <option key={e.id} value={e.id}>{e.nom} — {e.poste || 'Ouvrier qualifié'} — CHF {e.tarifJour}.-/j</option>)}
              </select></div>
            <div><label style={labelStyle}>Rôle</label>
              <select value={membreEquipe.role} onChange={e => setMembreEquipe({ ...membreEquipe, role: e.target.value })} style={inputStyle}>
                {["Chef de chantier", "Ouvrier qualifié", "Manœuvre", "Technicien", "Comptable", "Chef d'équipe", "Sous-traitant"].map(r => <option key={r}>{r}</option>)}
              </select></div>
            <div><label style={labelStyle}>Jours prévus</label>
              <input type="number" placeholder="10" value={membreEquipe.joursPlannifies} onChange={e => setMembreEquipe({ ...membreEquipe, joursPlannifies: e.target.value })} style={inputStyle} /></div>
            <button onClick={ajouterMembre} style={btnPrimaire}>+ Ajouter</button>
          </div>
