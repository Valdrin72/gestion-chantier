...
export const exportRapportQualite = async (chantier, qualiteData) => {
  const doc = new jsPDF();

  const checklistsDefaut = {
    demarrage: [
      { id: 'd1', categorie: 'Préparation', texte: 'Plans et documents reçus et vérifiés' },
      { id: 'd2', categorie: 'Préparation', texte: 'Réunion de démarrage effectuée' },
      { id: 'd3', categorie: 'Préparation', texte: 'Matériaux commandés et confirmés' },
      { id: 'd4', categorie: 'Sécurité', texte: 'EPI disponibles' },
      { id: 'd5', categorie: 'Sécurité', texte: 'Zone délimitée et signalisée' },
      { id: 'd6', categorie: 'Sécurité', texte: 'Accès sécurisé vérifié' },
      { id: 'd7', categorie: 'Protection', texte: 'Protection des zones adjacentes' },
    ...
    miParcours: [
      ...
      { id: 'm6', categorie: 'Qualité', texte: 'Étanchéité contrôlée' },
    ...
    reception: [
      ...
      { id: 'r3', categorie: 'Finitions', texte: 'Retouches effectuées' },
      ...
      { id: 'r9', categorie: 'Réception', texte: 'PV de réception signé' },
    ...
    securite: [
      ...
      { id: 's7', categorie: 'Accès', texte: 'Échafaudages sécurisés' },
      { id: 's8', categorie: 'Accès', texte: 'Zones à risque balisées' },
      { id: 's9', categorie: 'Urgence', texte: 'Trousse premiers secours disponible' },
      { id: 's10', categorie: 'Urgence', texte: "Numéros d'urgence affichés" },
    ],
  };

  const types = [
    { id: 'demarrage', label: '🚀 Démarrage', items: checklistsDefaut.demarrage, couleur: [33, 150, 243] },
    { id: 'miParcours', label: '🔍 Mi-parcours', items: checklistsDefaut.miParcours, couleur: [255, 152, 0] },
    { id: 'reception', label: '🏁 Réception', items: checklistsDefaut.reception, couleur: VERT },
    { id: 'securite', label: '🦺 Sécurité', items: checklistsDefaut.securite, couleur: ROUGE },
  ];

  let y = await ajouterEntete(doc, 'RAPPORT QUALITÉ', chantier.nom);
  ...
  doc.setFontSize(10);
  doc.setTextColor(...BLEU);
  doc.text(chantier.nom, 15, y + 6);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(9);
  doc.setTextColor(100, 100, 100);
  doc.text(`${chantier.numero || ''} · ${chantier.ville || ''} · ${chantier.statut || ''}`, 15, y + 12);
  doc.text(`Rapport généré le ${new Date().toLocaleDateString('fr-CH')}`, 200, y + 9, { align: 'right' });
  y += 18;

  // SCORES
  y = sectionTitre(doc, y, '📊 SCORES PAR TYPE DE CONTRÔLE');
  const scoreData = types.map(t => {
    const data = qualiteData[`${chantier.id}_${t.id}`] || {};
    const coches = t.items.filter(item => data[item.id]).length;
    const pct = Math.round((coches / t.items.length) * 100);
    return { ...t, coches, pct };
  });
  const totalCoches = scoreData.reduce((s, r) => s + r.coches, 0);
  const totalItems = types.reduce((s, t) => s + t.items.length, 0);
  const scoreGlobal = Math.round((totalCoches / totalItems) * 100);

  // BARRES DE SCORE
  scoreData.forEach((s, i) => {
    const x = 10 + (i * 47);
    doc.setFillColor(245, 245, 245);
    doc.rect(x, y, 43, 25, 'F');
    doc.setFillColor(...s.couleur);
    doc.rect(x, y + 18, 43 * (s.pct / 100), 4, 'F');
    doc.setFontSize(8);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(100, 100, 100);
    doc.text(s.label, x + 21, y + 7, { align: 'center' });
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(...(s.pct >= 80 ? VERT : s.pct >= 50 ? ORANGE : ROUGE));
    doc.text(`${s.pct}%`, x + 21, y + 16, { align: 'center' });
  });
  y += 30;

  // SCORE GLOBAL
  doc.setFillColor(...(scoreGlobal >= 80 ? [232, 245, 233] : scoreGlobal >= 50 ? [255, 243, 224] : [255, 235, 238]));
  doc.rect(10, y, 190, 14, 'F');
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(12);
  doc.setTextColor(...(scoreGlobal >= 80 ? VERT : scoreGlobal >= 50 ? ORANGE : ROUGE));
  doc.text(`SCORE GLOBAL : ${scoreGlobal}%  (${totalCoches}/${totalItems} points validés)`, 15, y + 9);
  doc.text(scoreGlobal >= 80 ? '✅ BON' : scoreGlobal >= 50 ? '⚠️ À AMÉLIORER' : '🔴 CRITIQUE', 195, y + 9, { align: 'right' });
  y += 18;

  // DÉTAIL PAR TYPE
  types.forEach(t => {
    if (y > 240) { doc.addPage(); y = 20; }
    y = sectionTitre(doc, y, t.label, t.couleur || BLEU);
    const data = qualiteData[`${chantier.id}_${t.id}`] || {};
    autoTable(doc, {
      startY: y,
      head: [['Point de contrôle', 'Catégorie', 'Statut']],
      body: t.items.map(item => [
        item.texte, item.categorie,
        data[item.id] ? '✅ Validé' : '❌ Non validé'
      ]),
      headStyles: { fillColor: t.couleur || BLEU, fontSize: 8 },
      bodyStyles: { fontSize: 8 },
      columnStyles: { 2: { halign: 'center', fontStyle: 'bold' } },
      didParseCell: (d) => {
        if (d.column.index === 2) {
          d.cell.styles.textColor = d.cell.raw === '✅ Validé' ? VERT : ROUGE;
        }
      },
      margin: { left: 10, right: 10 },
    });
  ...
  // NOTES
  const notes = qualiteData[`${chantier.id}_notes`];
  if (notes) {
    if (y > 240) { doc.addPage(); y = 20; }
    y = sectionTitre(doc, y, '📝 NOTES ET OBSERVATIONS', GRIS);
    doc.setFontSize(9);
  ...
  ajouterPiedPage(doc, false);
  doc.save(`Rapport_Qualite_${chantier.nom.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`);
};
