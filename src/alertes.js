...
export function calculerAlertes({ chantiers = [], devis = [], factures = [], paiements = {}, clients = [], chantiersStats = [] }, profilId = 'direction') {
  ...
  if (['direction', 'cyna', 'sinatec', 'conducteur', 'administratif'].includes(profilId)) {
    chantiers.forEach(c => {
  ...
  if (['direction', 'cyna', 'sinatec', 'administratif'].includes(profilId)) {
    devis.forEach(d => {
  ...
  if (['direction', 'cyna', 'sinatec', 'administratif'].includes(profilId) && chantiersStats.length > 0) {
