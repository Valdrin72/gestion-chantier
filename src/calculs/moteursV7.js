/**
 * Phase 7b — Moteurs V7 en PARALLÈLE des originaux (donnees.js).
 *
 * BUT : prouver, golden master à l'appui, que lire les pointages DIRECTEMENT
 * (via les helpers Phase 7a) donne EXACTEMENT le même résultat que lire le
 * journal dérivé. On ne remplace RIEN dans ce sprint : aucun consommateur
 * n'appelle ces fonctions. C'est la preuve avant la bascule (Phase 7c).
 *
 * Différence avec les originaux — UNIQUEMENT les lectures de chantier.journal :
 *   calculerCoutsChantier :
 *     - jours déplacement réels   : new Set(journal.map(date)).size  → joursReelsChantier
 *     - empIds avec heures        : journal.flatMap(...)             → empIdsChantier
 *     - heures par employé        : heuresEmploye(journal, id)       → heuresEmployeChantier
 *     - jours pour avancement     : new Set(journal.map(date)).size  → joursReelsChantier
 *   calculerEtatChantier :
 *     - empIds journal            : journal.flatMap(...)             → empIdsChantier
 *     - heures par employé        : heuresEmploye(journal, id)       → heuresEmployeChantier
 *     - totalJoursReels           : new Set(journal.map(date)).size  → joursReelsChantier
 *
 * Tout le reste (majorations CCT, CA, coûts matériel, projections, arrondis)
 * est STRICTEMENT identique aux originaux — les majorations viennent déjà des
 * pointages dans les deux versions (helper _surcoutMajorations recopié verbatim).
 */

import { calculerMajorationDate, calculerPartSemaine, facteurEffectif } from './majorations';
import { COEF_MO_DEFAUT } from './constants';
import { calculerCA, SEUILS } from '../donnees';
import { joursReelsChantier, heuresEmployeChantier, empIdsChantier } from './pointagesHelper';

// Réexport pour cohérence d'API si un futur consommateur en a besoin.
export { SEUILS };

/**
 * Copie VERBATIM de _surcoutMajorations (donnees.js L256). Cette fonction lit
 * DÉJÀ les pointages directement dans les deux moteurs originaux — la recopier
 * ici garantit un résultat identique sans toucher donnees.js.
 */
function _surcoutMajorationsV7(chantier, employes, pointages, coefficient) {
...
/**
 * V7 de calculerCoutsChantier — situation ACTUELLE, pointages lus directement.
 * Signature et sortie IDENTIQUES à l'original.
 */
export const calculerCoutsChantierV7 = (chantier, employes = [], localites = [], cfg = {}, devisList = [], pointages = []) => {
  ...
  // ⬇️ V7 : jours réels lus depuis les pointages (au lieu de new Set(journal.map(date)))
  ...
  // ⬇️ V7 : employés avec heures lus depuis les pointages (au lieu de journal.flatMap)
  ...
  const coutEquipeReelDetaille = [...empIdsAvecHeures, ...empIdsEquipeSansHeures].map(empId => {
    ...
    // ⬇️ V7 : heures lues depuis les pointages (au lieu de heuresEmploye(journal, id))
  ...
  const _maj = _surcoutMajorationsV7(chantier, employes, pointages, coefficient);
  ...
  // ⬇️ V7 : jours pour avancement lus depuis les pointages
...
/**
 * V7 de calculerEtatChantier — PROJECTION à fin de chantier, pointages lus directement.
 * Signature et sortie IDENTIQUES à l'original.
 */
export const calculerEtatChantierV7 = (chantier, employes = [], devisList = [], parametres = null, pointages = []) => {
  ...
  const _majE = _surcoutMajorationsV7(chantier, employes, pointages, coefficientMO);
