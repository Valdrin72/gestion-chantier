...
// ── APERÇU FINANCIER ────────────────────────────────────────────────────────
// « Résultat de période » = CA FACTURÉ HT de la période − DÉPENSES de la période (coûts au prorata).
// Numérateur ET dénominateur sur la même base de période (cohérent avec Marges/Finances). Garde-fou
// `vide` quand la période n'a aucune activité (au lieu d'un « CHF 0 » trompeur).
  ...
  if (vide) {
    return (
      <div style={{ ...carteV1, display: 'flex', flexDirection: 'column', height: '100%', minHeight: 180 }} data-testid="apercu-financier">
        <div style={{ ...mono(10, V1.texteMuted), textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 14 }}>RÉSULTAT DE PÉRIODE</div>
  ...
  return (
    <div style={{ ...carteV1, display: 'flex', flexDirection: 'column', height: '100%' }} data-testid="apercu-financier">
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 14 }}>
        <span style={{ ...mono(10, V1.texteMuted), textTransform: 'uppercase', letterSpacing: '0.1em' }}>RÉSULTAT DE PÉRIODE</span>
        ...
        <div style={{ flex: 1, minHeight: compact ? 140 : 120 }}>
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={serie} margin={{ top: 4, right: 4, bottom: 0, left: 4 }}>
              ...
              <Area type="monotone" dataKey="CA" stroke={V1.bleu} fill={V1.bleuFond} strokeWidth={2} name="Facturé" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
